...
|mission_platform |Platform type (see ICES controlled vocabulary http://vocab.ices.dk/?ref=311[here]). For example, define 'Research vessel' if your mission platform is a research vessel. |S | | |M |N
...
|ship_type |Describe type of ship that is hosting the acoustic instrumentation (see ICES controlled vocabulary http://vocab.ices.dk/?ref=311[here]). For example, define 'Research vessel' if it is a research vessel. |S | | |MA |1
|ship_code |For example, in-house code associated with ship, e.g. SS = Southern Surveyor or ship national identifier. |S | | |O |1
|ship_platform_code |ICES database of known ships (see http://vocab.ices.dk/Request/Login.aspx?ReturnUrl=%2frequest[here]). Requests can be made to add new vessels to the database by contacting accessions@ices.dk. For example, define '33OA' for the ship 'Oscar Dyson'. |S | |ICES/SeaDataNet |MA |1
|ship_platform_class |ICES controlled vocabulary for platform class (see http://vocab.ices.dk/?ref=311[here]). For example, define '31' for 'Research vessel'. |S | |ICES/SeaDataNet |MA |1
...
|platform_class |Controlled vocabulary for class of plaform (see ICES controlled vocabulary http://vocab.ices.dk/?ref=311[here]). |S | | |MA |1
